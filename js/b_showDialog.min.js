/** @auther:xylonhuang http://www.m.com/d/?p=313
@update:2011.10.24
@version:v1.1.9
**/
showDialog = (function () {
  ...
  NU = navigator.userAgent,
   isIE = ((NU.indexOf('MSIE') == -1) ? false : true),
    ...
    return { show: function (p) {
        var a = (typeof (p) === 'object') ? p : {}; this.id = a.id || null; this.bgcolor = a.bgcolor || '#111'; this.opacity = a.opacity || 70; 
        this.src = a.src || null; this.iWidth = a.iWidth || 'auto'; this.iHeight = a.iHeight || 'auto';
        this.sMsg = a.sMsg || null; this.sClass = a.sClass || null;
        this.sStyles = a.sStyle || 'padding:10px;border:4px solid red;background-color:#fff';
        ...
        var b = this.id, _src = this.src, c; if (!hasOverlay) {
            var c = gEl('_overlay_');
            if (c) {
                setStyle(c, { backgroundColor: "black", display: 'block' })
            }
            else {
                this.overlay()
            } 
        ...
        if (_src != null) {
            var d;
            if (!gEl(sIframeId)) {
                d = objDoc.createElement('div'); objOverLay.parentNode.insertBefore(d, objOverLay)
            } else {
        ...
        if (this.sMsg != null) {
            ...
            e.innerHTML = this.sMsg; if (!this.delayPop) {
                e.style.display = 'block'
            } 
            if (this.sTime != null) {
                var g = this; objWin.setTimeout(function () {
                    g.hide(e)
                }, this.sTime)
            } else {
                eMsgClose = function () {
        ...
        if (b != null && gEl(b)) {
            var h = gEl(b), _length = j.length; if (_length > 1) {
                gEl(j[_length - 2]).style.display = 'none'
            }
            h.style.position = 'absolute'; if (!!this.delayPop) {
                h.style.display = 'none'
            }
            else {
                h.style.display = 'block'
            } setStyle(h, {
                visibility: 'visible', zIndex: '9999', left: '50%', top: '50%'
            })
        }
        if (!this.delayPop) { this.adjust() }
    }, popup: function () {
        if (!!this.delayPop) {
            var a = this.id; if (this.src != null) {
                var b = gEl(sIframeId).parentNode; b.style.display = 'block'
            }
            if (a != null && gEl(a)) {
                var c = gEl(a); c.style.display = 'block'
            }
        } this.adjust()
    }, adjust: function () { 
                                                                              var a = this.id, _h = getWinHeight(), _rt = null, _st = null; if (a != null && gEl(a)) { var b = gEl(a), _iw = b.offsetWidth, _ih = b.offsetHeight; if (_ih >= _h) { longPop = true; setStyle(b, { position: 'absolute', top: '100px', marginLeft: '-' + _iw / 2 + 'px', marginBottom: '100px' }) } else { longPop = false; isIE6 ? (b.style.position = 'absolute') : (b.style.position = 'fixed'); setStyle(b, { marginTop: '-' + _ih / 2 + 'px', marginLeft: '-' + _iw / 2 + 'px' }) } fixOverlay = function () { if (_rt) { clearTimeout(_rt) } _rt = setTimeout(function () { showDialog.fix_Overlay() }, 250) }; addEvent(objWin, 'resize', fixOverlay); if (isIE || isCHROME) { this.fix_Overlay(); if (isIE6) { this.fixIE6_Center(); fixIECenter = function () { if (_st) { clearTimeout(_st) } _st = setTimeout(function () { showDialog.fixIE6_Center() }, 250) }; addEvent(objWin, 'scroll', fixIECenter) } } } }, fix_Overlay: function () { var a = getMaxW(), _h = getMaxH(), _wh = getWinHeight(), _obj = gEl(this.id), _ih; try { _ih = _obj.offsetHeight } catch (e) { _ih = this.iHeight; _obj = gEl(sIframeId) } if (_ih >= _wh) { longPop = true; setStyle(_obj, { position: 'absolute', top: '100px', marginTop: '0', marginBottom: '100px' }) } else { longPop = false } if (isIE) { objOverLay.style.width = a + 'px' } objOverLay.style.height = _h + 'px' }, fixIE6_Center: function () { if (!longPop) { var a = objBody.scrollTop || objDel.scrollTop, _obj = gEl(this.id); _obj.style.marginTop = parseInt(a - _obj.offsetHeight / 2) + 'px' } }, ifrAutoHeight: function (a) { if (this.iHeight != 'auto') { return } var b = gEl(a), h; try { var c = b.contentWindow.document.body.scrollHeight, _dh = b.contentWindow.document.documentElement.scrollHeight; h = Math.max(c, _dh); this.iHeight = b.height = h } catch (e) { } setStyle(b, { marginTop: '-' + b.offsetHeight / 2 + 'px', marginLeft: '-' + b.offsetWidth / 2 + 'px' }); this.fix_Overlay() }, hide: function (p) { var a = (typeof (p) === 'object') ? p : {}; this.cancleCallback = a.cancleCallback || false; var b = j[j.length - 1]; if (b == null) { return } if (b !== sPopupMsgId) { gEl(b).style.display = 'none' } else { objBody.removeChild(gEl(b)) } j = []; objOverLay.style.display = 'none'; hasOverlay = false; if (fixOverlay != null) { removeEvent(objWin, 'resize', fixOverlay) } if (eMsgClose != null) { removeEvent(objOverLay, 'click', eMsgClose) } if (isIE6 && fixIECenter != null) { removeEvent(objWin, 'scroll', fixIECenter) } var c = this.onCloseCallback; if (c != null && typeof (c) === 'function' && !this.cancleCallback) { c() } return }, overlay: function () { var a = getMaxH(); if (isIE) { var b = parseInt(getStyle(objBody, 'marginTop')), ib = parseInt(getStyle(objBody, 'marginBottom')); a = a + b + ib } objOverLay = objDoc.createElement('div'); objOverLay.setAttribute('id', '_overlay_'); setStyle(objOverLay, { backgroundColor: "black", position: 'absolute', height: a + 'px', zIndex: '9998', width: '100%', left: '0', top: '0' }); if (isIE6) { var c = getMaxW(); objOverLay.style.width = c + 'px'; objOverLay.innerHTML = '<iframe style="position:absolute;top:0;left:0;width:100%;height:100%;filter:alpha(opacity=0);z-index:9997" src="javascript:void(0)"></iframe>' } setOpacity(objOverLay, this.opacity); hasOverlay = true; return objBody.appendChild(objOverLay) } } })(); /*  |xGv00|485928a42275a027816ac4a7f1cc8299 */